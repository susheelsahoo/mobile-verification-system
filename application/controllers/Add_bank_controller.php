...
class Add_bank_controller extends CI_Controller {
     ...
     function get_description() {
        $array = array(
               'description'=> $this->input->post('value'));
                $this->load->model('Add_bank_model');
                $this->Add_bank_model->insert_description($array,$this->input->post('pk'));
            print_r($array);
    }
    ...
    function get_source() {
        $array = array(
               'source'=> $this->input->post('value'));
                $this->load->model('Add_bank_model');
                $this->Add_bank_model->insert_source($array,$this->input->post('pk'));
            print_r($array);
    }
    ...
    function get_bank_name() {
        $array = array(
               'bank_name'=> $this->input->post('value'));
                $this->load->model('Add_bank_model');
                $this->Add_bank_model->insert_bank_name($array,$this->input->post('pk'));
            print_r($array);
    }
    ...
    public function bank_validation()
    {
        try {
            $this->load->library('form_validation');
            ...
            if ($this->form_validation->run()) {
                ...
                $this->load->model('Add_bank_model');
                $insert_id = $this->Add_bank_model->insert_bank_dropdwon($array);
                if ($insert_id) {
                    $response = array(
                        'success' => true,
    ...
    function fetch_all_bank()
    {
        try {
            $this->load->model("Add_bank_model");
            $fetch_case = $this->Add_bank_model->make_datatables_bank_dropdown();
            $data = array();
            foreach ($fetch_case as $row) {
                $sub_array = array();
                // $buttons = '';
            ...
            $output = array(
                "draw" => intval($_POST["draw"]),
                "recordsTotal" => $this->Add_bank_model->get_all_data_bank_dropdown(),
                "recordsFiltered" => $this->Add_bank_model->get_filtered_data_bank_dropdown(),
                "data" => $data
            );
            echo json_encode($output);
        } catch (Exception $ex) {
